...
export const Route = createFileRoute("/my-pc")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Live system telemetry from your gaming PC: usage, temperatures, network and streaming stats." },
    ],
  }),
  component: MyPcPage,
});
  ...
  return (
    <AppLayout>
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="glass inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-semibold text-primary">
      ...
      <section className="mt-8">
        ...
        <GlassCard className="grid gap-6 p-6 md:grid-cols-3">
          <StatMeter label={t("mypc.cpuUsage")} display={`${tel.cpuUsage}%`} percent={tel.cpuUsage} icon={<Cpu className="h-4 w-4" />} />
          <StatMeter label={t("mypc.gpuUsage")} display={`${tel.gpuUsage}%`} percent={tel.gpuUsage} icon={<MonitorPlay className="h-4 w-4" />} tone="cool" />
          <StatMeter label={t("mypc.ramUsage")} display={`${tel.ramUsage}%`} percent={tel.ramUsage} icon={<MemoryStick className="h-4 w-4" />} />
        </GlassCard>
      </section>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-2">
        <section>
          ...
          <div className="grid grid-cols-2 gap-4">
            <InfoTile icon={Thermometer} accent label={t("mypc.cpuTemp")} value={`${tel.cpuTempC}°C`} />
            <InfoTile icon={Thermometer} accent label={t("mypc.gpuTemp")} value={`${tel.gpuTempC}°C`} />
            ...
            <InfoTile
              ...
              sub={tel.batteryPercent === null ? "Desktop" : tel.batteryCharging ? "Charging" : "On battery"}
            />
          </div>
        </section>
        ...
        <GlassCard className="p-6">
          <div className="mb-5 flex items-center justify-between">
            ...
            <span className="inline-flex items-center gap-2 rounded-full bg-primary/15 px-3 py-1 text-sm font-semibold text-primary">
              <span className="h-2 w-2 rounded-full bg-primary animate-pulse-ring" />
              {t("mypc.streaming.active")}
            </span>
          </div>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            <InfoTile icon={MonitorPlay} label={t("mypc.resolution")} value={streamStatus.resolution} />
            <InfoTile icon={Gauge} label={t("mypc.fps")} value={`${streamStatus.fps}`} />
            <InfoTile icon={Radio} label={t("mypc.bitrate")} value={`${streamStatus.bitrateMbps} Mbps`} />
            <InfoTile icon={Film} label={t("mypc.codec")} value={streamStatus.codec} />
            <InfoTile icon={Wifi} label={t("mypc.quality")} value={qualityLabel} />
            <InfoTile icon={Radio} label={t("mypc.latency")} value={`${streamStatus.latencyMs} ms`} />
            <InfoTile icon={ArrowDownToLine} label={t("mypc.packetLoss")} value={`${streamStatus.packetLossPercent}%`} />
            <InfoTile
              icon={Gamepad2}
              accent
